...
(function (Modules) {
  ...
  Modules.AppStepNav = function () {
    ...
    this.start = function ($element) {
      ...
      function bindToggleForSteps (stepTracker) {
        $element.find('.js-toggle-panel').click(function (event) {
          ...
          var toggleClick = new StepToggleClick(event, stepView, $steps, stepTracker, stepIsOptional)
          toggleClick.track()
      ...
      function bindComponentLinkClicks (stepTracker) {
        $element.find('.js-link').click(function (event) {
          var linkClick = new componentLinkClick(event, stepTracker, $(this).attr('data-position')) // eslint-disable-line no-new, new-cap
          linkClick.track()
      ...
      function bindToggleShowHideAllButton (stepTracker) {
        $showOrHideAllButton = $element.find('.js-step-controls-button')
        $showOrHideAllButton.on('click', function () {
          ...
          if ($showOrHideAllButton.text() === actions.showAllText) {
            ...
            stepTracker.track('pageElementInteraction', 'stepNavAllShown', {
            ...
            stepTracker.track('pageElementInteraction', 'stepNavAllHidden', {
    ...
    function StepView ($stepElement) {
      ...
      function setIsShown (checkShown) {
        $stepElement.toggleClass('step-is-shown', checkShown)
        $stepContent.toggleClass('js-hidden', !checkShown)
        $titleLink.attr('aria-expanded', checkShown)
        $stepElement.find('.js-toggle-link').html(checkShown ? actions.hideText : actions.showText)
      }
